...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    >
        ...
        <header class="p-5 flex items-center gap-3">
            ...
            <h1 class="text-2xl font-semibold">Laporan Keamanan</h1>
        </header>
        <main class="px-5">
            <div class="flex flex-col gap-1">
                <p class="text-xl font-semibold">{{$laporanKeamanan->title}}</p>
                
            </div>
            <hr class="my-10">

            <div class="flex flex-col gap-1 mb-5">
                <p>{{$laporanKeamanan->satpam->user->name}}</p>
                <p>NIP: {{$laporanKeamanan->satpam->nip}}</p>
            </div>
            ...
            {{-- <div class="my-10 flex flex-col gap-1">
                <p>{{date('j F Y', strtotime($laporanKeamanan->created_at))}}</p>
                @php
                    $ketua = App\Models\PengurusKeamanan::with('paguyuban')->where('paguyuban_id', $laporanPaguyuban->paguyuban->id)->where('posisi', 'ketua')->first();
                    @endphp
