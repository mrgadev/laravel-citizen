    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="mb-4 flex gap-1 sm:mb-0">
                ...
                <span class="material-symbols-rounded">chevron_right</span>
                <a href="#" class=" flex items-center gap-1 text-blue-600"><span class="material-symbols-rounded">person_add</span> Edit User</a>
            </div>
        ...
        <div class="col-span-full xl:col-span-8 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
            ...
            <form action="{{route('user.update', $user->id)}}" method="POST" enctype="multipart/form-data" class="p-3">
                @csrf
                @method('PUT')
                <img src="{{Storage::url($user->image)}}" class="w-36 h-36 rounded-full mb-3" alt="">
                <div class="mb-3 grid md:grid-cols-3 gap-5">
                    <div>
                        <x-label for="image" value="{{ __('Foto Profil') }}" />
                        <input class="w-full rounded outline-none border-1 border-gray-300" id="image" type="file" name="image"  autofocus /> 
                    </div>
                    <div>
                ...
                <div class="mb-3 grid md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <select name="role" id="role" class="w-full rounded outline-none border-1 border-gray-300">
                            <option value="{{$user->role}}">{{ucfirst($user->role)}} (Selected)</option>
