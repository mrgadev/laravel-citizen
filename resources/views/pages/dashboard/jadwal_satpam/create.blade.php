    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="mb-4 flex gap-1 sm:mb-0">
                ...
                <span class="material-symbols-rounded">chevron_right</span>
                <a href="{{route('satpam.jadwal.index', $satpam->id)}}" class=" flex items-center gap-1"><span class="material-symbols-rounded">account_circle</span> Jadwal untuk {{$satpam->user->name}}</a>
                <span class="material-symbols-rounded">chevron_right</span>
        ...
        <div class="col-span-full xl:col-span-8 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
            ...
            <form action="{{route('satpam.jadwal.store')}}" method="POST" enctype="multipart/form-data" class="p-3">
                @csrf
                @method('POST')
                <div class="mb-3 grid md:grid-cols-3 gap-5">
                    <x-input id="satpam_id" value="{{$satpam->id}}" type="hidden" name="satpam_id"  required autofocus /> 
                    <div>
                        <x-label for="tanggal" value="{{ __('Tanggal') }}" />
                        <x-input id="tanggal" type="date" name="tanggal"  required autofocus /> 
                    </div>
                    <div>
                        <x-label for="jam_mulai" value="{{ __('Jam Mulai') }}" />
                        <x-input id="jam_mulai" type="time" name="jam_mulai"  required autofocus /> 
                    </div>
                    <div>
                        <x-label for="jam_selesai" value="{{ __('Jam Selesai') }}" />
                        <x-input id="jam_selesai" type="time" name="jam_selesai"  required autofocus /> 
                    </div>
                </div>
