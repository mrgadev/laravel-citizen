    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="col-span-full xl:col-span-8 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
            <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex flex-col gap-2 md:flex-row justify-between items-center">
                <h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-100">Tambah Data Paguyuban</h2>
                
            </header>
            {{-- <x-input-error></x-input-error> --}}
            <form action="{{route('paguyuban.store')}}" method="POST" enctype="multipart/form-data" class="p-3">
                @csrf
                @method('POST')
                <div class="grid md:grid-cols-3 gap-3 mb-5">
                    <div>
                        <x-label for="nama" value="{{ __('Nama') }}" />
                        <input class="rounded-md border-1 border-gray-400 w-full" id="nama" type="text" name="nama" :value="old('nama')" required autofocus /> 
                    </div>
                    <div >
                        <x-label for="email" value="{{ __('Email') }}" />
                        <input class="rounded-md border-1 border-gray-400 w-full" id="email" type="email" name="email" :value="old('email')" required autofocus />                
                    </div>
                    <div >
                        <x-label for="telepon" value="{{ __('Telepon') }}" />
                        <input class="rounded-md border-1 border-gray-400 w-full" id="telepon" type="text" name="telepon" :value="old('telepon')" autofocus />                
                    </div>
                </div>
                <div >
                    <x-label for="alamat" value="{{ __('Alamat Lengkap') }}" />
                    <textarea name="alamat" id="alamat" class="w-full rounded-md outline-none border-1 border-gray-400"></textarea>
                </div>
