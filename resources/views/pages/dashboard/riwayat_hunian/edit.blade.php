    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="col-span-full xl:col-span-8 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
            <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex flex-col gap-2 md:flex-row justify-between items-center">
                <h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-100">Edit Riwayat Hunian</h2>
                
            </header>
            {{-- <x-input-error></x-input-error> --}}
            <form action="{{route('riwayat_hunian.update', $riwayatHunian->id)}}" method="POST" enctype="multipart/form-data" class="p-3">
                @csrf
                @method('PUT')
                <div class="grid md:grid-cols-2 gap-5 mb-5">
                    <div class="">
                        ...
                        <select name="hunian_id" id="hunian_id" class="w-full rounded outline-none border-1 border-gray-200">
                            <option value="{{$riwayatHunian->hunian_id}}">{{$riwayatHunian->hunian->nama}} (Selected)</option>
                    ...
                    <div >
                        ...
                        <select name="warga_id" id="warga_id" class="w-full rounded outline-none border-1 border-gray-200">
                            <option value="{{$riwayatHunian->warga_id}}">{{$riwayatHunian->warga->nama}} (Selected)</option>
                ...
                <div class="grid md:grid-cols-3 gap-3 mb-5">
                    <div>
                        ...
                        <select name="status" id="status" class="w-full rounded outline-none border-1 border-gray-200">
                            <option value="{{$riwayatHunian->status}}">{{$riwayatHunian->status}} (Selected)</option>
                    ...
                    <div >
                        ...
                        <input class="w-full rounded outline-none border-1 border-gray-200" id="tanggal_mulai" type="date" name="tanggal_mulai" value="{{$riwayatHunian->tanggal_mulai}}" required autofocus />                
                    </div>
                    <div >
                        <x-label for="tanggal_akhir" value="{{ __('Tangal Akhir') }}" />
                        <input class="w-full rounded outline-none border-1 border-gray-200" id="tanggal_akhir" type="date" name="tanggal_akhir" value="{{$riwayatHunian->tanggal_akhir}}" autofocus />                
                    </div>
                </div>
