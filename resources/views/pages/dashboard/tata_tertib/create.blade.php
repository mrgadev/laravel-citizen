<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="col-span-full xl:col-span-8 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
            ...
            <form action="{{route('tata_tertib.store')}}" method="POST" enctype="multipart/form-data" class="p-3">
                @csrf
                @method('POST')
                <div class="my-3">
                    <x-label for="judul" value="{{ __('Judul') }}" />
                    <input class="rounded-md border-1 border-gray-400 w-full" id="judul" type="text" name="judul" :value="old('judul')" required autofocus /> 
                </div>
                <input type="hidden" value="{{auth()->user()->id}}" name="user_id">
                <div class="my-3">
                    {{-- <x-label for="isi" value="{{ __('Judul') }}" /> --}}
                    <textarea name="isi" id="isi" cols="30" rows="30"></textarea>
                </div>
